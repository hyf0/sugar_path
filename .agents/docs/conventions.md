...
The checked-in allocation snapshots hard-gate successful `alloc`, `alloc_zeroed`, and `realloc` call counts for warmed named operations. Requested bytes remain target-specific evidence and may change without failing the check. When an intended optimization changes counts, regenerate every affected target and feature-configuration snapshot and explain which execution path changed; the current matrix contains three targets with default and Rolldown configurations. Do not update a snapshot merely to make CI green.
